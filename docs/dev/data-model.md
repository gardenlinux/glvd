...
The value of the "debsec_vulnerable" is determined [here](https://github.com/gardenlinux/glvd-data-ingestion/blob/4574003f767c10fd9ac029f303f0826e10c24112/src/glvd/cli/data/combine_deb.py#L56) as part of the ingestion process.
...
### `cve_context_kernel`
Kernel-specific CVE context.
This information is derived from the upstream kernel's `vulns` repo.
We only record data for kernels used in Garden Linux, i.e. only LTS kernels.
We have an ignore list for kernel subsystems that are not considered relevant, either because they are not compiled into our kernels or because it is unlikely to affect typical Garden Linux use-cases.
...
| is_fixed              | boolean | True if in any version                      |
| is_relevant_subsystem | boolean | True if relevant subsystem for Garden Linux |
| source_data           | jsonb   | Raw source data                             |

---

### `image_variant`
Tracks Garden Linux images.
This is required so users can filter CVEs by their image and ignore CVEs not applicable to their image.
...
See [glvd-data-ingestion schema/README.md](https://github.com/gardenlinux/glvd-data-ingestion/blob/main/schema/README.md) for details on how to add new migrations.
