...
if kubectl --namespace glvd get po | grep -q init-pg ; then
    ...
    kubectl --namespace glvd run init-pg --image=ghcr.io/gardenlinux/glvd-init:2025.11.12 --restart=Never --env=PGHOST=glvd-database-0.glvd-database --env=PGPASSWORD="$DB_PASSWORD"
